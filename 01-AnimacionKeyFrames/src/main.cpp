...
// Shader con multiples texturas ejercicio
...
void init(int width, int height, std::string strTitle, bool bFullScreen) {
	...
	glTextureParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	if (data) {
		glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, imageWidth, imageHeight, 0, GL_BGRA, GL_UNSIGNED_BYTE, data);//BGRA es el formato interno de la imágen
		//Generar los niveles de los mipmap
		glGenerateMipmap(GL_TEXTURE_2D);
	}
	else {
